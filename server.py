...
def createAccount():
    ...
    query = "INSERT INTO users (userGroup, userName, userPassword) VALUES ('%s', '%s', '%s');" % (
    userGroup, userName, userPassword)
    dbCur.execute(query)
    dbConnection.commit()
    response.status = 200
    return 'Выполнено'
...
def checkDb():
    ...
    # If db don't exist create tables and add default data
